...
### Get Data
```bash
docling https://arxiv.org/pdf/2408.13296 --to text
...
mkdir -p ./paper/input
mv 2408.13296v3.txt  ./paper/input

# 1. setup workspace
graphrag init --root ./paper

# 2. running the pipeline
graphrag index --root ./paper

# methods: local|global|drift|basic

# 3. running the query: local
graphrag query \
--root ./paper \
--method local \
--query "How does a company choose between RAG, fine-tuning, and different PEFT approaches?"

# 4. running the query: global
graphrag query \
--root ./paper \
--method global \
--query "How does a company choose between RAG, fine-tuning, and different PEFT approaches?"

# 5. running the query: drift
graphrag query \
--root ./paper \
--method drift \
--query "How does a company choose between RAG, fine-tuning, and different PEFT approaches?"
